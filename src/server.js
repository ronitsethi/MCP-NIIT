...
export function createServer() {
    ...
    for (const [key, content] of Object.entries(ALL_RESOURCES)) {
        ...
        server.resource(key, new ResourceTemplate(uri, { list: undefined }), {
            description,
    ...
    logger.info(`Server configured: ${Object.keys(ALL_RESOURCES).length} resources, 15 tools registered.`);
    return server;
}
/**
 ...
 */
function buildResourceDescription(content) {
    const lines = [
        content.overview,
        "",
    ...
    if (content.relatedTools && content.relatedTools.length > 0) {
        lines.push("", `WHEN THIS IS INSUFFICIENT — USE THESE TOOLS INSTEAD: ${content.relatedTools.join(", ")}`);
    }
    return lines.join("\n");
}
