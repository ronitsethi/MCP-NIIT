...
export function registerSurveyTools(server) {
    server.tool("reopen_survey_window", `Reopen or extend a post-delivery survey access window for a specific course offering.
...
- "Reopen survey access for Leadership Essentials until end of month"`, ReopenSurveyWindowSchema.shape, async (input) => {
        logger.info("Tool called: reopen_survey_window", input);
        const result = await reopenSurveyWindow(input);
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
    });
    server.tool("get_survey_report", `Fetch the latest survey participation metrics or report artifacts for a course offering.
...
- "Get me the detailed survey results for offering OFF-2001"`, GetSurveyReportSchema.shape, async (input) => {
        logger.info("Tool called: get_survey_report", input);
        const result = await getSurveyReport(input);
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
    });
    server.tool("amend_survey_closure", `Request a change to the survey closure date for a specific offering.
...
- "Extend the survey deadline by one week"`, AmendSurveyClosureSchema.shape, async (input) => {
        logger.info("Tool called: amend_survey_closure", input);
        const result = await amendSurveyClosure(input);
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
    });
    server.tool("get_express_interest_report", `Fetch the express interest (demand capture) report for a specific course.
...
- "Pull the express interest report for the last 6 months"`, GetExpressInterestReportSchema.shape, async (input) => {
