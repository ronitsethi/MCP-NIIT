...
export function registerOfferingTools(server: McpServer) {
  server.tool(
    "create_offering_request",
    `Submit a request to create a new future offering for a course.
...
- "No upcoming sessions — need a face-to-face offering for Project Management"`,
    CreateOfferingRequestSchema.shape,
    async (input) => {
...
- "Move OFF-2002 from June to July and change it to VILT"`,
  ...
  server.tool(
    ...
    `Fetch future offerings for a course, including dates, locations, status, and capacity.
...
- "Are there any available sessions for this course?"`,
    GetUpcomingOfferingsSchema.shape,
    async (input) => {
