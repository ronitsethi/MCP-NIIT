...
export function registerSupportTools(server: McpServer) {
  server.tool(
    "resolve_technical_access_issue",
    `Log and route a learner-facing technical access issue across enterprise systems (Moodle, SharePoint, MS Teams, Workday, or other).
...
- "Learner is getting permission denied on the learning platform"`,
    ResolveTechnicalAccessIssueSchema.shape,
    async (input) => {
