...
export function registerLoiTools(server: McpServer) {
  server.tool(
    "process_loi_request",
    `Process a Letter of Intent (LOI) related support or workflow request.
...
- "LOI cannot save data and draft is not generating"`,
    ProcessLoiRequestSchema.shape,
    async (input) => {
