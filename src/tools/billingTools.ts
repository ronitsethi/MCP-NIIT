...
export function registerBillingTools(server: McpServer) {
  server.tool(
    "check_course_billing_info",
    `Fetch billing-related information for a course, filtered by participant type and billing scenario.
...
- "Confirm billing owner for Leadership Essentials"`,
    CheckCourseBillingInfoSchema.shape,
    async (input) => {
