...
export function registerCourseTools(server: McpServer) {
  server.tool(
    "get_course_fee_info",
    `Return fee information for a specific course, optionally filtered by participant type and billing scenario.
...
- "Get fee details for Project Management Foundations"`,
    GetCourseFeeInfoSchema.shape,
    async (input) => {
