...
export function registerEnrollmentTools(server: McpServer) {
  server.tool(
    ...
    `Fetch current enrollment numbers, capacity, and waitlist status for a specific offering.
...
- "Check enrollment for offering OFF-2003"`,
    GetEnrollmentStatusSchema.shape,
    async (input) => {
