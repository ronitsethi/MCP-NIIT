...
export async function bookTrainingRoom(input) {
    const result = await query(`SELECT r.* 
     ...
     LIMIT 1`, [`%${input.location.toLowerCase()}%`, input.capacity, input.sessionDate]);
    ...
    await query(`INSERT INTO bookings (booking_id, room_id, booking_date, requested_by, course_id, booking_reference, status)
     VALUES ($1, $2, $3::date, $4, $5, $6, 'confirmed')`, [bookingId, matchingRoom.room_id, input.sessionDate, input.requestedBy, input.courseId ?? null, bookingReference]);
...
export async function getRoomBookings(input) {
    const conditions = [];
    const params = [];
    let paramIndex = 1;
    if (input.date) {
    ...
    const result = await query(`SELECT b.booking_id, b.room_id, r.location, r.capacity, 
     ...
     ${whereClause}
     ORDER BY b.booking_date, r.location`, params);
